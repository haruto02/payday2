...
if string.lower(RequiredScript) == "lib/setups/setup" then
	...
	GameInfoManager._TIMER_CALLBACKS = {
		...
		overrides = {
			...
			[102452] = { pause = function(...) GameInfoManager._TIMER_CALLBACKS.overrides.stop_on_pause(...) end }, --Mountain Master Achiev Timer
            [103872] = { pause = function(...) GameInfoManager._TIMER_CALLBACKS.overrides.stop_on_pause(...) end }, --Mountain Master Achiev Timer Alt
		}
	}
	...
	GameInfoManager._INTERACTIONS = {
		INTERACTION_TO_CALLBACK = {
			...
			mex_pickup_murky_uniforms = 		"_special_equipment_interaction_handler",
...
if string.lower(RequiredScript) == "lib/player_actions/skills/playeractiontagteam" then
	...
	local function GetUnitName(unit)
		...
		elseif unit:in_slot(16) or unit:in_slot(24) then	-- Bot (mask on/off)/Joker
			...
			name = VHUDPlus:getCharacterName(tweak_id, false)
		end
		return name
	end
