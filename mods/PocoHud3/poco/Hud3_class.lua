...
local O, K, L, me, blendMode, interactionCircleAlpha, buffO, floatO, popupO
PocoHud3Class = {
	ALTFONT	= ALTFONT	,
	FONT		= FONT		,
	FONTLARGE = FONTLARGE,
...
function THitDirection:init(owner, data)
	...
	local color = data.shield and math.lerp( Opt.shieldColor, Opt.shieldColorDepleted, rate ) or math.lerp( Opt.healthColor, Opt.healthColorDepleted, rate )
...
function PocoHud3Class._drawHeistStats(tab)
	...
	local addJob = function(host, heist, jobData)
	...
	local jobs = tweak_data.narrative.jobs
	for _, contact in ipairs(contacts_sorted) do
		for _, job_index in ipairs(jobs_index) do
...
function PocoHud3Class._drawKit(tab)
	...
	draw = function()
		...
		local __, lbl = _.l({font=FONT, color=cl.LightSteelBlue, font_size=20, pnl = pnl, x = 20, y = y(0)},
			L('{_Chapter} {_kit_save_desc1} {_RC|White} {_kit_save_desc2|White}'),true)
		y(lbl:h())
		...
		elem = PocoUIKeyValue:new(tabEdt,{ pnl = pnl,
			...
			hintText = L('{_kit_key_edit_hint1}  {_kit_key_edit_hint2|Yellow} {_kit_key_edit_hint3}\n{_kit_key_edit_hint4|Silver}\n{_kit_key_edit_hint5|Red}')
		})
		_Current['key'] = {elem}
		...
		__, lbl = _.l({font=FONT, color=cl.LightSteelBlue, font_size=20, pnl = pnl, x = 20, y = y(0)},
			L('{_Chapter} {_kit_saved_kits_title1} {_RC|White} {_kit_saved_kits_title2|White}'),true)
		y(lbl:h())
			...
			for col,category in pairs(categories) do
				if col ~= 1 then
					...
					row[#row+1] = PocoUIButton:new(tabEdt,{ pnl = pnl,
						...
						x = 0, y = 0, w=150, h=22,
						fontSize = 22, text={ind,cl[K:get(ind,'color') or 'White']}, hintText = L('{_kit_equip_btn_hint1} {_kit_equip_btn_hint2|Tan}\n{_kit_equip_btn_hint3} {_kit_equip_btn_hint4|Red} {_kit_equip_btn_hint5}')
					})
				end
			end
		...
		local titles = L('{_Chapter} {_kit_profiler_desc1} {_RC|White} {_kit_profiler_desc2|White}')
	...
	PocoUIButton:new(tabEdt,{
		...
		text={L('_word_save'),cl.CornFlowerBlue},hintText = L('{_kit_save_hint1|Tan} / {_kit_save_hint2|Tomato} {_kit_save_hint3}')
	})
	tabEdt.pnl:bitmap({
		texture = 'guis/textures/pd2/shared_lines',	wrap_mode = 'wrap',
